#!/usr/bin/env python3
"""Creates JPEG grid on processed images for the jpeg_quality dataset"""
...
def main():
    ...
    for folder in tbar:
        if (folder / 'jpeg_quality_0.png').exists() and (
                folder / 'jpeg_quality_1.png').exists():
            continue
        ...
        with open(folder / 'gt_jpeg_quality.txt', 'r') as f:
            quality0, = map(int, f.readline().split())
            quality1, δy, δx = map(int, f.readline().split())

        # create img_0
        buffer = io.BytesIO()
        Im.save(buffer, "JPEG", quality=quality0)
        img_0 = np.array(Image.open(buffer))
        imageio.imwrite(folder / 'jpeg_quality_0.png', img_0)
        ...
        imageio.imwrite(folder / 'jpeg_quality_1.png', img_1)
